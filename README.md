...
智能语义压缩：集成 Gemini 1.5 Flash，自动过滤网页杂质，产出“三句要点总结 + 分段中英对照”的高质量内容。
...
复制配置模板并修改 config/config.ini。

设置临时环境变量并运行项目：

# 设置秘钥（本地测试环境）
export GEMINI_API_KEY="您的_Gemini_Key"
export SMTP_PASSWORD="您的邮箱应用专用密码"
...
私有仓库：将本项目推送到您的 GitHub Private Repository。

配置加密秘钥：在仓库设置 Settings -> Secrets and variables -> Actions 中添加：

GEMINI_API_KEY: Google AI Studio 申请的 API 密钥。

SENDER_EMAIL: 用于发送邮件的邮箱。

SMTP_PASSWORD: 邮箱生成的 App Password。
...
手动触发测试：在 Actions 标签页手动运行一次 RSS Intelligence Daily。
...
|  | MaxArticlesPerRun | 单次运行 AI 处理的最大文章数 | 12 |
| AI | ModelName | 使用的 Gemini 模型版本 | gemini-1.5-flash |
|  | RequestDelay | 两次 AI 请求间的间隔（秒），避免触发 API 限制 | 4 |
| SMTP | Server | 发件服务器 SMTP 地址 (如 https://www.google.com/search?q=smtp.gmail.com) | smtp.gmail.com |
...
如果需要调整 AI 的总结风格（例如要求更硬核的技术分析或更感性的阅读摘录），请直接修改 main.py 中的 IntelligenceHub.process 函数内的 prompt 内容。